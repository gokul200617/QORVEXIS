...
@router.get("/summary")
def get_provider_summary(db: Session = Depends(get_db)):
    """Return the high-level multi-provider dashboard payload."""
    try:
        return provider_summary_service.get_summary(db)
...
def get_provider_comparison(db: Session = Depends(get_db)):
    """Return the provider comparison matrix."""
    try:
        return provider_intelligence_service.get_provider_comparison(db)
    except Exception as exc:
...
def get_provider_recommendations(db: Session = Depends(get_db)):
    ...
    try:
        return provider_optimization_engine.generate_recommendations(db)
    except Exception as exc:
...
def get_provider_workloads(db: Session = Depends(get_db)):
    """Return the workload attribution mapping (spend & latency per signature)."""
    try:
        return provider_optimization_engine.get_workload_attributions(db)
    except Exception as exc:
...
def get_provider_visibility(db: Session = Depends(get_db)):
    """Return the visibility and data source capability report."""
    try:
        return provider_visibility_service.get_visibility_report(db)
    except Exception as exc:
...
@router.post("/sync")
def sync_provider_usage(db: Session = Depends(get_db)):
    """Trigger an immediate sync of provider usage."""
    try:
        return provider_usage_sync_service.sync_all(db)
    except Exception as exc:
